...
class _AddServicePageState extends State<AddServicePage> {
  ...
  Future<void> sendANewService() async {
    showCustomProgressBar(context);
    try {
      void clearOperation() {
        ...
        showToastMobile(msg: "You have added a new service!");
      }

      void afterAllPostHasBeenAdded(
          Map<String, dynamic> serviceData, String categoryId) {
        CategoryNotifier()
            .processServicesData(categoryId, NotifierType.normal, serviceData);
        clearOperation();
        closeCustomProgressBar(context);
        Navigator.pop(context);
      }
      ...
      // Send a new post
      final services = await ServicesOperation().sendANewService(
          serviceIdentity,
          serviceDescription,
          servicePrice,
          categoryId,
      ...
      if (services != null) {
        // Part id
        String servicesId = services[dbReference(Services.id)];
        ...
        if (getMediaBytes.isNotEmpty) {
          ...
          final uploads = getMediaBytes
              ...
              .map((index, mediaByte) => MapEntry(
                  index,
                  CategoryOperation().uploadPostMedia(servicesId, index,
          ...
          if (uploadOperation.isNotEmpty) {
            afterAllPostHasBeenAdded(services, categoryId);
          } else {
        ...
        // Post was not added
        showToastMobile(msg: "An error with service addition");
        closeCustomProgressBar(context);
      }
