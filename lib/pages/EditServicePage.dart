...
class EditServicePage extends StatefulWidget {
  ...
  const EditServicePage({super.key, required this.servicesData});
...
class _EditServicePageState extends State<EditServicePage> {
  ...
  void initState() {
    ...
    identityController.text = widget.servicesData.servicesIdentity;
    descriptionController.text = widget.servicesData.servicesDescription;
    priceController.text = widget.servicesData.servicesPrice;
    categoryNotifier.sendNewState(CategoryNotifier()
        .getCategoryData(widget.servicesData.servicesCategoryId));
    currencyNotifier.sendNewState(widget.servicesData.servicesCurrency);
    mediaNotifier.sendNewState([...(widget.servicesData.servicesMedia), null]);
    ...
    if (getMediaBytes.isNotEmpty) {
      ...
      if (uploadOperation.isNotEmpty) {
        ...
        CategoryNotifier()
            ...
            ?.updateThePartMediaData(widget.servicesData.servicesId, [
  ...
  Future<void> updateAService() async {
    showCustomProgressBar(context);
    try {
      void clearOperation() {
        ...
        showToastMobile(msg: "You have updated service!");
      }

      void afterAllPostHasBeenAdded(
        Map<String, dynamic> serviceData,
      ) {
        CategoryNotifier().updateServicesData(
            widget.servicesData.servicesCategoryId,
            NotifierType.normal,
            serviceData);
        clearOperation();
        closeCustomProgressBar(context);
      ...
      // Get Identity
      String serviceIdentity = identityController.text.toString().trim();
      // Get Identity
      String serviceDescription = descriptionController.text.toString().trim();
      // Get Price
      String servicePrice = priceController.text.toString().trim();
      // Get Category
      ...
      // Send a new post
      final services = await ServicesOperation().updateAService(
          serviceIdentity,
          serviceDescription,
          servicePrice,
          currency,
      ...
      if (services != null) {
        afterAllPostHasBeenAdded(services);
      } else {
        // Post was not added
        showToastMobile(msg: "An error with service update");
        closeCustomProgressBar(context);
      }
                    ...
                    Padding(
                      ...
                      child: MultiWidgetStateConsumer(
                          ...
                          widgetStateListBuilder: (context) {
                            bool enable = identityNotifier.currentValue ==
                                    true ||
                                ...
                                currencyNotifier.currentValue !=
                                    widget.servicesData.servicesCurrency;

                            return CustomPrimaryButton(
                                isEnabled: enable,
                                buttonText: "Update",
                                onTap: updateAService);
                          }),
                    ),
